...
class SampleScene(BaseScene):
    ...
    def _draw_header(self, img: Image.Image, draw: ImageDraw.ImageDraw, alpha: float) -> None:
        ...
        draw.rounded_rectangle(
            [
                label_x - badge_pad,
                label_y - badge_pad // 2,
                label_x + lw + badge_pad,
                label_y + label_size + badge_pad // 2,
            ],
            radius=label_size // 3,
            fill=badge_fill,
        ...
        sep_color = (
            tuple(max(0, c - 30) for c in self.theme.bg_color)
            if self.theme.bg_color[0] > 128
            else tuple(min(255, c + 25) for c in self.theme.bg_color)
        )
        draw.line([(pad, sep_y), (self.width - pad, sep_y)], fill=sep_color, width=1)

    # ── Body: karaoke transcript ──────────────────────────────────────────────

    def _draw_body(self, draw: ImageDraw.ImageDraw, local_t: float, alpha: float) -> None:
        if not self.words:
    ...
    def _draw_placeholder(self, draw: ImageDraw.ImageDraw, local_t: float, alpha: float) -> None:
    ...
    def _draw_progress(self, draw: ImageDraw.ImageDraw, local_t: float, alpha: float) -> None:
        ...
        # Track background
        track_color = tuple(
            max(0, c - 20) if c > 128 else min(255, c + 20) for c in self.theme.bg_color
        )
        ...
        if fill_w > 0:
            draw.rectangle([pad, y, pad + fill_w, y + bar_h], fill=self.theme.progress_color)
        ...
        cw = cb[2] - cb[0]
        draw.text(
            (self.width - pad - cw, y - int(self.height * 0.022)),
            counter,
            font=f_counter,
            fill=counter_color,
        )
    ...
    def _draw_waveform(self, draw: ImageDraw.ImageDraw, local_t: float) -> None:
        frame_idx = min(int(local_t * self.fps), len(self.waveform_frames) - 1)
